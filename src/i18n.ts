...
const DICT = {
  ...
  'tour.desktop.1.title': { en: 'Choose how you want to inspect the shoot', zh: '上方：切換工作視角', ja: '上部：作業ビューを切り替える' },
  'tour.desktop.1.body': { en: 'Studio, Top plan, Viewfinder and Photo render answer different questions. Arrange in Studio or Top plan, frame in Viewfinder, then validate the result in Photo render.', zh: '「棚內」看高度與陰影、「俯視」排燈距離、「取景」確認構圖，最後才進入「照片渲染」檢查成品。', ja: 'スタジオで高さと影、俯瞰図で距離、ファインダーで構図を確認し、最後にフォトレンダーで仕上がりを確認します。' },
  'tour.desktop.2.title': { en: 'Left: build and select the scene', zh: '左側：建立與選取場景物件', ja: '左側：シーンを作成・選択する' },
  'tour.desktop.2.body': { en: 'Add lights, people, props, grip and backdrops here. Selecting Camera, a subject or a light immediately changes the controls shown on the right.', zh: '在這裡新增燈具、人物、道具、控光附件與背景。點選 Camera、人物或燈具後，右側控制面板會立即切換成對應設定。', ja: 'ライト、人物、小道具、グリップ、背景を追加します。カメラ、人物、ライトを選ぶと、右側の設定が対象に合わせて切り替わります。' },
  ...
  'tour.desktop.4.title': { en: 'Right: refine the selected object', zh: '右側：精修目前選取的物件', ja: '右側：選択中の対象を調整する' },
  'tour.desktop.4.body': { en: 'This panel follows your selection. For a light, adjust power, fixture, modifier, colour, position and target; for a camera, adjust lens, focus and exposure.', zh: '這個面板會跟隨左側選取內容。選燈時可調功率、燈具、塑光、色彩、位置與目標；選相機時則調鏡頭、對焦與曝光。', ja: '左側の選択に連動します。ライトでは出力、灯体、モディファイア、色、位置、照射先、カメラではレンズ、ピント、露出を調整します。' },
  'tour.desktop.5.title': { en: 'Measure exposure instead of guessing', zh: '測光：用數據找出曝光問題', ja: '測光：数値で露出の問題を見つける' },
  'tour.desktop.5.body': { en: 'Open Exposure Analysis for EV, histogram, false colour and clipping. The real before/after above shows the analysis panel appearing over the same scene.', zh: '開啟測光分析即可查看 EV、直方圖、假色與剪裁警示。上方是真實操作前後，同一場景打開分析面板後的變化。', ja: '露出分析で EV、ヒストグラム、フォルスカラー、白飛びを確認します。上は同じシーンで分析を開く前後の実画面です。' },
  ...
  'tour.desktop.7.title': { en: 'Render only when the setup is ready', zh: '渲染：把即時預覽變成最終照片', ja: 'レンダー：ライブ表示を最終写真へ' },
  'tour.desktop.7.body': { en: 'Photo render switches from the fast planning view to progressive path tracing. Compare the real before/after, then wait for noise to settle before exporting PNG.', zh: '「照片渲染」會從快速規劃畫面切換到漸進式光線追蹤。可用上方真實前後畫面比較，等雜訊穩定後再輸出 PNG。', ja: 'フォトレンダーで高速プレビューからプログレッシブ・パストレーシングへ切り替えます。ノイズが落ち着いてから PNG を出力します。' },
  ...
  'tour.mobile.2.title': { en: 'Subject tab: pose and appearance', zh: '人物分頁：調整姿勢與外觀', ja: '人物タブ：ポーズと外観' },
  ...
  'tour.mobile.3.title': { en: 'Light tab: tune one light at a time', zh: '燈光分頁：逐盞調整燈具', ja: 'ライトタブ：1 灯ずつ調整' },
  ...
  'tour.mobile.4.title': { en: 'Camera tab: frame the shot', zh: '相機分頁：完成取景與曝光', ja: 'カメラタブ：構図と露出' },
  ...
  'tour.mobile.5.title': { en: 'Project tab: protect and share the work', zh: '專案分頁：保存、備份與分享', ja: 'プロジェクトタブ：保存・共有' },
  'tour.mobile.5.body': { en: 'Save locally, export a portable backup, import another project or copy a review link. Important work should always have an exported backup.', zh: '可立即儲存、匯出可攜備份、匯入專案或複製分享連結；重要作品務必另存一份備份檔。', ja: 'ローカル保存、バックアップ書き出し、読み込み、共有リンクのコピーができます。重要な作品は必ず書き出してください。' },
