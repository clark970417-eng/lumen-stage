...
const COPY: Record<Locale, Copy> = {
  zh: {
    ...
    supportTitle: '支援與回報', supportBody: ['遇到載入、儲存、算圖或裝置相容問題時，請先重新載入網站，並確認瀏覽器已更新。', '回報問題時，請附上裝置、瀏覽器版本、畫面尺寸、重現步驟與截圖；不要附上私人照片或未公開專案檔。', 'LUMEN STAGE 目前由 YuYing 維護。公開問題與功能建議可透過 GitHub Issues 提交。'], issues: '前往 GitHub Issues', back: '返回首頁',
  },
  en: {
    ...
    supportTitle: 'Support', supportBody: ['For loading, storage, rendering or compatibility problems, reload first and confirm your browser is current.', 'Include device, browser version, viewport, reproduction steps and a screenshot. Do not attach private photographs or unreleased project files.', 'LUMEN STAGE is maintained by YuYing. Public bugs and feature requests can be filed through GitHub Issues.'], issues: 'Open GitHub Issues', back: 'Back to home',
  },
  ja: {
    ...
    supportTitle: 'サポート', supportBody: ['読み込み、保存、レンダリング、互換性の問題は、再読み込みとブラウザ更新を最初に確認してください。', '端末、ブラウザ版、画面サイズ、再現手順、スクリーンショットを添えてください。非公開の写真やプロジェクトは添付しないでください。', 'LUMEN STAGE は YuYing が管理しています。公開バグと要望は GitHub Issues から送信できます。'], issues: 'GitHub Issues を開く', back: 'ホームへ戻る',
  },
}
...
function SiteFooter({ copy }: { copy: Copy }) {
  return <footer className="site-footer"><span>© {new Date().getFullYear()} YuYing · LUMEN STAGE</span><nav><a href="/privacy">{copy.privacyTitle}</a><a href="/terms">{copy.termsTitle}</a><a href="/support">{copy.supportTitle}</a><a href="https://github.com/clark970417-eng/lumen-stage" target="_blank" rel="noreferrer">GitHub</a></nav></footer>
}
