    ...
    <aside className="inspector panel">
      ...
      {isLight && light && <>
        <section className="inspector-section">
          ...
          <label className="select-row fixture-select"><span>{t('light.profile')}</span><select aria-label={t('light.profile')} value={light.profileId} onChange={(event) => { const profileId = event.target.value as keyof typeof LIGHT_PROFILES; const profile = LIGHT_PROFILES[profileId]; state.updateLight(light.id, { profileId, headType: profile.headType, operationMode: profile.headType === 'strobe' ? 'flash' : 'continuous' }) }}>{Object.values(LIGHT_PROFILES).map((profile) => <option key={profile.id} value={profile.id}>{profile.maker} · {profile.model}</option>)}</select></label>
          <Range label={t('light.power')} value={light.powerPercent} min={1} max={100} step={1} unit="%" onChange={(value) => state.updateLight(light.id, { powerPercent: value })} />
      ...
      {modifier && <section className="inspector-section grip-inspector">
        ...
        <div className="coordinate-label"><span>{t('axis.posAndAngle')}</span><small>METERS / DEG</small></div>
      ...
      {studioObject && <section className="inspector-section studio-object-inspector">
        ...
        <div className="object-type-grid" role="group" aria-label={t('object.typeAria')}>{(['subject','chair','table','plinth','cube','sphere'] as const).map((type) => <button key={type} className={studioObject.type === type ? 'active' : ''} onClick={() => state.updateStudioObject(studioObject.id, { type })}>{t(`object.${type}`)}</button>)}</div>
