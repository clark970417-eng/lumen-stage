    ...
    <aside className="inspector panel">
      ...
      {isLight && light && <>
        <InspectorDrawer key="light-output" title={t('light.section')} meta="LIGHT / OUTPUT" action={<button onClick={state.resetLighting}>{t('light.resetAll')}</button>}>
          ...
          <Range label={t('light.power')} value={lightWattage(light)} min={1} max={wattageLimit(light)} step={1} unit={light.operationMode === 'flash' ? ' Ws' : ' W'} onChange={(value) => state.updateLight(light.id, { powerPercent: percentForWattage(light, value) })} />
