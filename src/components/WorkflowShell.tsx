...
const COPY = {
  en: {
    ...
    mainSubject: 'Main subject', camera: 'Camera A', layout: 'Position layout', addItem: 'Add to stage', removeItem: 'Remove selected item', shots: 'shots', key: 'Key light', fill: 'Fill light', rim: 'Separation light', background: 'Background light', effect: 'Effect light',
    ...
    square: 'Soft key', round: 'Round source', strip: 'Strip source', person: 'Person', dog: 'Dog', cat: 'Cat', product: 'Product', chair: 'Chair', table: 'Table', plinth: 'Product plinth', cube: 'Cube', sphere: 'Sphere', fabric: 'Fabric swatch', choosePerson: 'Choose a person', choosePersonHint: 'Select the starting body frame. You can refine appearance later.', female: 'Woman', male: 'Man', back: 'Back', reflector: 'Reflector', flag: 'Black flag', vflat: 'V-Flat',
  ...
  zh: {
    ...
    mainSubject: '主要人物', camera: '相機 A', layout: '全場位置', addItem: '新增到舞台', removeItem: '刪除選取項目', shots: '個鏡位', key: '主光', fill: '補光', rim: '分離光', background: '背景光', effect: '效果光',
    ...
    square: '柔光主燈', round: '圓形光源', strip: '條形光源', person: '人物', dog: '狗狗', cat: '貓咪', product: '商品', chair: '椅子', table: '桌子', plinth: '商品台', cube: '方塊', sphere: '球體', fabric: '布料樣本', choosePerson: '選擇人物', choosePersonHint: '先選擇起始骨架，加入後仍可調整外觀。', female: '女性', male: '男性', back: '返回', reflector: '反光板', flag: '黑旗', vflat: 'V-Flat',
  ...
  ja: {
    ...
    mainSubject: 'メイン人物', camera: 'カメラ A', layout: '全体配置', addItem: 'ステージに追加', removeItem: '選択中の項目を削除', shots: 'ショット', key: 'キーライト', fill: 'フィルライト', rim: 'セパレーション', background: '背景ライト', effect: 'エフェクト',
    ...
    square: 'ソフトキー', round: '円形光源', strip: 'ストリップ', person: '人物', dog: '犬', cat: '猫', product: '商品', chair: '椅子', table: 'テーブル', plinth: '展示台', cube: 'キューブ', sphere: '球体', fabric: '生地サンプル', choosePerson: '人物を選択', choosePersonHint: '最初の体型を選びます。外観は追加後も調整できます。', female: '女性', male: '男性', back: '戻る', reflector: 'レフ板', flag: '黒旗', vflat: 'V-Flat',
...
export function WorkflowNavigation() {
  ...
  const isActive = (id: WorkflowStage) => id === 'intent'
    ? stage === 'intent' || stage === 'blocking'
    : id === 'framing'
      ? stage === 'framing' || stage === 'verify'
      : stage === id
...
export function BlueprintPanel() {
  ...
  const planning = stage === 'intent' || stage === 'blocking'
  const shooting = stage === 'framing' || stage === 'verify'
  ...
  return <aside className="blueprint-panel panel">
    ...
    <section className="blueprint-shot">
      <span>{copy.shot} {String(studio.shots.length + 1).padStart(2, '0')}</span>
    ...
    {shooting && <section className="blueprint-section"><h2>{copy.framing}</h2><button className="blueprint-row active" onClick={() => studio.selectObject('camera')}><i className="camera-icon" /><span><strong>{copy.camera}</strong><small>{studio.focalLength} mm · ƒ/{studio.aperture} · ISO {studio.iso}</small></span></button></section>}
    ...
    {stage === 'layout' && <section className="blueprint-section layout-section">
      <header className="layout-section-heading">
        <h2>{copy.layout}</h2>
        <div className="layout-section-actions">
          <button className="layout-add-button" aria-label={copy.addItem} title={copy.addItem} onClick={() => setAssetOpen(true)}>＋</button>
...
export function AssetDrawer() {
  ...
  const tabs: ReadonlyArray<'lights' | 'people' | 'grip' | 'backdrops'> = mode === 'layout'
    ...
    : mode === 'lighting' ? ['lights', 'grip'] : ['people', 'backdrops']
  useEffect(() => {
  ...
  return <div className="workspace-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }}><section className="asset-drawer" role="dialog" aria-modal="true" aria-label={copy.assetTitle}>
    ...
    <div className="asset-drawer-grid">
      ...
      {tab === 'people' && !personPicker && <>{([['subject', copy.person], ['dog', copy.dog], ['cat', copy.cat], ['product', copy.product], ['chair', copy.chair], ['table', copy.table], ['plinth', copy.plinth], ['cube', copy.cube], ['sphere', copy.sphere], ['fabric', copy.fabric]] as const).map(([id, label]) => <button key={id} onClick={() => id === 'subject' ? setPersonPicker(true) : finish(() => studio.addStudioObject(id))}><i className={`studio-object-icon ${id}`} /><strong>{label}</strong><small>STAGE OBJECT</small></button>)}</>}
