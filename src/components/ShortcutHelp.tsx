...
export function ShortcutLauncher() {
  ...
  return (
    <div className="shortcut-launcher">
      <button className={open ? 'active' : ''} onClick={() => setValue('shortcutHelpOpen', !open)} title={t('shortcuts.launcher.title')}>
        {t('shortcuts.launcher')} <kbd>?</kbd>
      </button>
    </div>
  )
}
...
export function ShortcutHelp() {
  ...
  return (
    <div className="shortcut-overlay" role="dialog" aria-modal="true" aria-label={t('shortcuts.title')} onClick={(event) => { if (event.target === event.currentTarget) setValue('shortcutHelpOpen', false) }}>
      <div className="shortcut-dialog">
        <header>
          <div><strong>{t('shortcuts.title')}</strong><small>KEYBOARD SHORTCUTS</small></div>
          <button ref={closeButton} aria-label={t('shortcuts.close')} onClick={() => setValue('shortcutHelpOpen', false)}>×</button>
        </header>
                    ...
                    <ul>
                      {items.map((shortcut) => (
                        <li key={shortcut.id} className={shortcut.available && !shortcut.available(state) ? 'unavailable' : ''}>
                          <span className="shortcut-label">{t(shortcut.label)}{shortcut.hint && <small>{t(shortcut.hint)}</small>}</span>
        ...
        <footer>
          <span>{t('shortcuts.unavailable')}</span>
          <span>{t('shortcuts.closePre')} <kbd>Esc</kbd> {t('shortcuts.closeSep')} <kbd>?</kbd> {t('shortcuts.closePost')}</span>
        </footer>
      </div>
    </div>
  )
